...
async function TaskList(){
 return new Promise((resolve) => {
   $.get(taskUrl(`task/list`,{"modelId":"M10001","plateCode":1}),async(error, response, data) =>{
    try{
      if (error) {
        ...
        if(result.code!=='0'){
          ...
          TaskInfo = result.result.taskInfoList
      ...
      try {
        if (err) {
          ...
          if(result.code!=='0'){
            ...
            // 去做任务
            console.log(`📝去做任务：【${result.result.taskName}】 - 任务奖励【${result.result.awardValue}】🥔鲜豆 - 待领取奖励💰`)
          }
        }} catch (e) {
        console.log(e)
      } finally {
